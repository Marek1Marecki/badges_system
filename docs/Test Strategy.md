...
| **Container & Runtime Security** | `Trivy` | Analiza końcowa. Skanuje w 100% zbudowany **Obraz Dockera**. Weryfikuje pakiety systemu (Debian) oraz wirtualnego środowiska Pythona. Pełni rolę bramki Gatingu blokującej dziurawe obrazy. Dodatkowo narzędzie to, w osobnym kroku potoku, **generuje manifest SBOM (Software Bill of Materials)** w formacie CycloneDX dla celów audytowych. | **Blokująca.** Weryfikacja następuje *po* zbudowaniu obrazu (Build), a *przed* testami integracyjnymi. |
