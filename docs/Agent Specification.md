...
4. **Mutacje i Agregaty (Niemutowalność Operacyjna):** Żadna z klas domenowych (Encji ani Agregatów, takich jak `TouristProfileDomain` czy `BadgeVersionDomain`) nie może bezpośrednio mutować swoich atrybutów. Proces aktualizacji stanu biznesowego (np. zmiana nazwy, awans na plan PRO) musi przebiegać z wykorzystaniem metod fabrykujących lub operatora `replace` (dataclasses), zwracających zawsze nową instancję zaktualizowanego obiektu.
5. **Rejestrowanie Faktów (Audit Trail & Event Sourcing):** Każda operacja zmiany stanu agregatu o znaczeniu krytycznym (np. `BadgeStatusChanged`, `ProfileUpdated`, `AscentLogged`) musi wygenerować `DomainEvent`, dołączany do kolejki zdarzeń powiązanej z agregatem (np. za pomocą metody `events()`). Zakazu się jawnego wstrzykiwania infrastrukturalnego modelu `AuditLog` do warstwy Czystej Domeny i Warstwy Aplikacji. Zdarzenia te konsumowane są przez `CeleryEventPublisher`, który samodzielnie dokonuje trwałego zapisu w bazie danych z zachowaniem zasady *Append-Only* na wypadek kasacji RODO.
...
9. **Delegacja Logiki Wielokrotnego Użytku (Application Services):** Orkiestratory (Use Case'y) nie mogą powielać złożonej logiki walidacyjnej (np. sprawdzania ram bitemporalnych T-01 i T-03). Jeśli logika powtarza się w wielu Use Case'ach (np. pojedynczy log wejścia vs masowy import GPX), należy wydzielić ją do Serwisu Aplikacyjnego (np. `BitemporalValidationService`) wstrzykiwanego przez kontener DI.
...
13. **Zasada Izolacji Fabryk (Factory Pattern):** Repozytoria danych (np. `DjangoBadgeRepository`) mają za zadanie wyłącznie komunikację z ORM. Kategorycznie zakazuje się umieszczania w nich logiki instancjonowania obiektów domenowych na podstawie skomplikowanych danych z JSONB. Proces ten musi zostać oddelegowany do dedykowanych klas fabryk (np. `infrastructure/factories/badge_rule_factory.py`), które parsują słowniki i zwracają czyste obiekty domenowe.
...
4. **Zasada Płaskiej Hierarchii Geograficznej (Ltree):** Kategorycznie zakazuje się tworzenia nowych modeli w Django ORM dla jednostek podziału terytorialnego (np. `CountyModel`, `CityModel`). Zgodnie z `ADR-026`, wszystkie regiony geograficzne i administracyjne żyją w zunifikowanym modelu `RegionFlatModel`. Nowe poziomy należy definiować wyłącznie jako wpisy w enumeratorze `RegionLevel`. Wszelkie relacje sąsiedztwa realizowane są przez zunifikowane pole `neighbors` typu `ManyToManyField("self")`.
