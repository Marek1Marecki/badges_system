...
**Rozwiązanie wdrożone (2026-09-03):**
- ✅ `VerifyBadgeUseCase` (`EvaluateBadgeProgressQuery`) → `VerifyBadgeResponseDTO` (z `TierResultResponseDTO`)
- ✅ `BulkLogAscentsUseCase` → `BulkAscentResultDTO` (istniało, było nieużywane — teraz używane)
- ✅ `ExploreMapUseCase` → `MapExploreResponseDTO` (z `GeoJSONFeatureDTO`)
- ✅ Kontrolery API używają `result.model_dump()` (BadgeProgressView, BulkAscentLogView, MapExploreView)
- ✅ 5 DTOs oznaczonych jako nested w `test_dto_naming_convention.py`

**Pozostałe (tech debt):**
- [ ] `OsmRepositoryPort.fetch_multiple_from_osm()` — `dict[str, OsmNodeData]` → własny DTO

**Weryfikacja:** 854 testów, 80.92% cov, 5/5 lint-imports KEPT, mypy OK

**Komentarz Architekta:**
Zjawisko *Primitive Obsession* rozwiązane dla 3 głównych Use Case'ów. Pozostał `dict[str, OsmNodeData]` w adapterze OSM — wymaga refaktoryzacji DaneWejściowe/OsmNodeData jako DTO (niska widoczność).
